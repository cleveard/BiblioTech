...
class BookTagDaoTest {
    ...
    private suspend fun doTestTagAddUpdateDelete() {
        ...
        // Add the tags for the books, again
        tags = addTags(books, emptyArray())
        assertWithMessage("Delete selected tags").apply {
            ...
            that(bookTagDao.queryBookIds(books.map { it.book.id }.toTypedArray())?.toSet()).isEqualTo(
                setOf(tags[1].id, tags[2].id)
            )
            // Do it again and verify that nothing was deleted
            ...
            // Delete by Id
            that(db.getTagDao().deleteWithUndo(tags[1].id)).isEqualTo(1)
            // Verify that the tags were deleted
            tagList = tagDao.getLive().getLive()
            that(tagList?.size).isEqualTo(1)
            that(tagList!![0]).isEqualTo(tags[2])
            that(bookTagDao.queryBookIds(books.map { it.book.id }.toTypedArray())?.toSet()).isEqualTo(
                setOf(tags[2].id)
            )
            // Do it again and verify nothing was deleted
            that(db.getTagDao().deleteWithUndo(tags[1].id)).isEqualTo(0)
            // Verify that the tags were deleted
            tagList = tagDao.getLive().getLive()
            that(tagList?.size).isEqualTo(1)
            that(tagList!![0]).isEqualTo(tags[2])
            ...
            // Delete by Id
            that(db.getTagDao().deleteWithUndo(tags[2].id)).isEqualTo(1)
            // Verify that the tags were deleted
            tagList = tagDao.getLive().getLive()
            that(tagList?.size).isEqualTo(0)
            that(bookTagDao.queryBookIds(books.map { it.book.id }.toTypedArray())?.toSet()?.size).isEqualTo(0)
            // Do it again and verify nothing was deleted
            that(db.getTagDao().deleteWithUndo(tags[2].id)).isEqualTo(0)
            // Verify that the tags were deleted
            tagList = tagDao.getLive().getLive()
            that(tagList?.size).isEqualTo(0)
            that(bookTagDao.queryBookIds(books.map { it.book.id }.toTypedArray())?.toSet()?.size).isEqualTo(0)
