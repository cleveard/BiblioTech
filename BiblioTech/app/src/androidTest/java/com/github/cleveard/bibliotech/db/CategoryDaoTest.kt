...
class CategoryDaoTest {
    ...
    private suspend fun doTestAddUpdateDelete() {
        ...
        suspend fun addCategories() {
            ...
            for (c in categories) {
                assertWithMessage("Add %s", c.category).apply {
                    ...
                    // Add an category
                    c.id = 0L
                    categoryDao.addWithUndo(book.book.id, c)
                    // Check the id
            ...
            categoryDao.addWithUndo(books[0].book.id, c)
        ...
        assertWithMessage("Delete keep categories").apply {
            ...
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id), false)).isEqualTo(2)
            ...
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id), false)).isEqualTo(0)
            // Verify that the categories were kept
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(3)
            // Delete the categories for books[0]. Check that one book-category links are deleted
            that(categoryDao.deleteWithUndo(arrayOf(books[1].book.id), false)).isEqualTo(1)
            // Verify that the categories were kept
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(3)
        }
        ...
        assertWithMessage("Delete delete categories").apply {
            // Delete the categories for books[0]. Check that two book-category links are deleted
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id))).isEqualTo(2)
            // Verify that the categories were deleted
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(1)
            that(categoryList!![0]).isEqualTo(categories[1])
            // Do it again and verify that nothing was deleted
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id))).isEqualTo(0)
            // Verify that the categories were deleted
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(1)
            that(categoryList!![0]).isEqualTo(categories[1])
            // Delete the categories for books[0]. Check that one book-category links are deleted
            that(categoryDao.deleteWithUndo(arrayOf(books[1].book.id))).isEqualTo(1)
            categoryList = categoryDao.get()
            // Verify that the categories were deleted
        ...
        assertWithMessage("Delete filtered keep categories").apply {
            ...
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id), false, filter))
                .isEqualTo(0)
            // Verify that the categories were kept
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(3)
            that(categoryDao.deleteWithUndo(arrayOf(books[1].book.id), false, filter))
                .isEqualTo(1)
            // Verify that the categories were kept
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(3)
        ...
        assertWithMessage("Delete filtered delete categories").apply {
            // Delete the categories for books[0]. Check that nothing is delete, because the filter doesn't match
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id), true, filter))
                .isEqualTo(0)
            // Verify that the categories were kept
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(3)
            // Delete the categories for books[0]. Check that one book-category links are deleted, because the filter doesn't match
            that(categoryDao.deleteWithUndo(arrayOf(books[1].book.id), true, filter))
                .isEqualTo(1)
            // Verify that the categories were deleted
            categoryList = categoryDao.get()
        ...
        categoryDao.addWithUndo(books[0].book.id, categories)
        assertWithMessage("Add categories from array").apply {
            ...
            categoryDao.addWithUndo(books[1].book.id, categories)
            // Verify that they are there
            categoryList = categoryDao.get()
            that(categoryList?.size).isEqualTo(3)
            // Delete books[0] categories
            that(categoryDao.deleteWithUndo(arrayOf(books[0].book.id))).isEqualTo(3)
            ...
            that(categoryDao.deleteWithUndo(arrayOf(books[1].book.id))).isEqualTo(3)
