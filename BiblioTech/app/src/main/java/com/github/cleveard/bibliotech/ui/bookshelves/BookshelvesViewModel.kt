...
internal class BookshelvesViewModel(private val app: Application) : AndroidViewModel(app) {
            ...
            while (lbs != null || gbs != null) {
                ...
                when {
                    lbs?.bookshelf?.bookshelfId == gbs?.bookshelfId -> {
                        ...
                        refreshShelfAndBooks(auth, lbs!!, gbs!!, false)
    ...
    private suspend fun refreshBooksOnShelves(auth: BookCredentials, shelf: BookshelfEntity, tag: TagEntity) {
        ...
        // Get the set of volume ids on the shelf in google books we last received
        val lastShelfSet: Set<String> = repo.getShelfVolumeIds(shelf.id)
        // The volume ids of the books tagged by the linked tag the last time we merged
        val lastDbSet: Set<String> = repo.getLastTagVolumeIds(tag.id)
        ...
        val conflicts = tagInDb.filterKeys { it in deleteFromShelf } +
            addToShelf.filterKeys { it in untagInDb }.apply {
                values.forEach { it.book.isSelected = true }
            }
        ...
        tagInDb.values.forEach {book ->
            ...
            // Add this volumeId to the last tagged books, if needed
            if (!lastDbSet.contains(book.book.volumeId)) {
                repo.addLastTagVolume(tag.id, GoogleBookLookup.kSourceId, book.book.volumeId!!)
            }
        }
        ...
        untagInDb.forEach {volumeId ->
            ...
            repo.getBookIdByVolumeId(GoogleBookLookup.kSourceId, volumeId)?.let {
                ...
                // Update the last tagged books
                if (lastDbSet.contains(volumeId))
                    repo.removeLastTagVolume(tag.id, GoogleBookLookup.kSourceId, volumeId)
            }
        }

        newInShelf.values.forEach {
            repo.addLastShelfVolume(shelf.id, it.book.volumeId!!)
        }

        missingFromShelf.forEach {
            repo.removeLastShelfVolume(shelf.id, it)
        }
    ...
    suspend fun refreshShelfAndBooks(auth: BookCredentials, shelf: BookshelfAndTag, forceBookRefresh: Boolean) {
        ...
        withContext(Dispatchers.IO) {
            ...
            repo.withUndo(app.applicationContext.getString(R.string.refresh_one_book, shelf.bookshelf.title)) {
                refreshShelfAndBooks(auth, shelf, it, forceBookRefresh)
            }
        }
    }
     ...
     */
    private suspend fun refreshShelfAndBooks(auth: BookCredentials, shelf: BookshelfAndTag, from: BookshelfEntity, forceBookRefresh: Boolean = false) {
        ...
        shelf.tag?.let {tag ->
            if (forceBookRefresh || from.booksModified > shelf.bookshelf.booksLastUpdate) {
                refreshBooksOnShelves(auth, shelf.bookshelf, tag)
