...
class AutoFitPreviewBuilder private constructor(
    ...
    init {
        ...
        useCase.setOnPreviewOutputUpdateListener(Preview.OnPreviewOutputUpdateListener {
            ...
            val parent = subViewFinder.parent as ViewGroup
            parent.removeView(subViewFinder)
            parent.addView(subViewFinder, 0)

            // Update internal texture
            subViewFinder.surfaceTexture = it.surfaceTexture

            // Apply relevant transformations
            bufferRotation = it.rotationDegrees
            val rotation = getDisplaySurfaceRotation(subViewFinder.display)
            updateTransform(subViewFinder, rotation, it.textureSize, viewFinderDimens)
        })

        // Every time the provided texture view changes, recompute layout
        viewFinder.addOnLayoutChangeListener { view, left, top, right, bottom, _, _, _, _ ->
            val subViewFinder = view as TextureView
            val newViewFinderDimens = Size(right - left, bottom - top)
            Log.d(TAG, "View finder layout changed. Size: $newViewFinderDimens")
            val rotation = getDisplaySurfaceRotation(subViewFinder.display)
            updateTransform(subViewFinder, rotation, bufferDimens, newViewFinderDimens)
        }
